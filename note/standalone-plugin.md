...
1. **完整实机运行验证**：已确认编译通过、打包完整，全部 16 个 YAML 经解析校验合法，并经过一轮针对运行期正确性的代码审查（修复了一个会导致所有机器注册失败的构造期 NPE 等共 11 处问题，见 git log）；但未在真实 MC 服务器中实机加载验证（本环境无法运行服务端），需在装好前置的服务器实测一次。
2. **作物破坏掉落**：已成熟作物破坏掉落作物/种子（`getDrops` 覆盖为空，仅由 CropListener 按成熟度掉落，对齐 wt_crop.js）；未成熟破坏不掉落（同原脚本）。
